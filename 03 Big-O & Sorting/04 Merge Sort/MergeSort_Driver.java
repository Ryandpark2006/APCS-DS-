...
class SelectionLowHigh
{
   ...
   {  
      for(int x = high; x > low; x--)
         swap(array, findMax(array, low, x), x); 
   }
   private static int findMax(double[] array, int low, int upper)  //change bounds upper + 1  max int is low     = low
   {
      double max = array[low];
      int index = low;     //have to stay within bounds
      for(int x = low; x < upper + 1; x++)
         if(array[x] > max)
         {
            max = array[x];
            index = x;
