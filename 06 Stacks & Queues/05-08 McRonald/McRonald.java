...
public class McRonald
{
   ...
   public static void mcRonald(int TIME, PrintWriter of)
   {
      ...
      **********************************/
      ...
      for(int x = 0; x < TIME; x++)
      {
         if(Math.random() <= 0.2)
         {
            line.add(new Customer(x, timeToOrderAndBeServed()));
            customers += 1;
            displayTimeAndQueue(line, x);
            if(line.size() == 1)
            {
               thisCustomersTime = line.peek().getOrderAndBeServed();
               of.println("\t" + line.peek() + " is now being served for " + thisCustomersTime + " minutes.");
            }
            else
               of.println("\t" + line.peek() + " is now being served for " + thisCustomersTime + " minutes.");
         }
         ...
         if(thisCustomersTime != 0 && thisCustomersTime != -1)
            thisCustomersTime -= 1;
         if(thisCustomersTime == 0)
         {
            arrivalTime = line.peek().getArrivedAt();
            line.remove();
            waitTime = x - arrivalTime;
            totalMinutes += waitTime;
            longestWaitTime = Math.max(longestWaitTime, waitTime);
            longestQueue = Math.max(longestQueue, line.size());
            if(line.peek() != null)
               thisCustomersTime = line.peek().getOrderAndBeServed();
            else
               thisCustomersTime = -1;   
      ...
      while(!line.isEmpty())
      {
         ...
         if(thisCustomersTime == 0)
         {
            arrivalTime = line.peek().getArrivedAt();
            line.remove();
            waitTime = 1080 - arrivalTime + timePassed;
            totalMinutes += waitTime;
            longestWaitTime = Math.max(longestWaitTime, waitTime);
            longestQueue = Math.max(longestQueue, line.size());
            if(line.peek() != null)
    ...
    ***********************************/
      public Customer(int timeArrived, int servingTime){
         arrivedAt = timeArrived;
         orderAndBeServed = servingTime;
      }
      ...
      public String toString(){
         return Integer.toString(arrivedAt);  //not sure what to return here
      }
