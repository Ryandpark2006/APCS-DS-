...
public class LunchRoom
{
   ...
   public static void lunchRoom(int TIME, PrintWriter outfile)
   {
      ...
      *************************/
      ...
      int juniorLongest = 0; 
      int sophomorTotal = 0;
      double sophomorWait = 0;
      int sophomorLongest = 0; 
      int freshmanTotal = 0;
      double freshmanWait = 0;
      int freshmanLongest = 0; 
      int thisCustomersTime = -1;
      int timePassed = 0;
      ...
      for(int x = 0; x < TIME; x++)
      {
         if(Math.random() <= 0.2)
         {
            ...
            if(chance <= 0.25)
            {
               line.add(new Customer(x, "Se", timeToOrderAndBeServed()));
            }
            if(chance > 0.25 && chance <= 0.5)
            {
               line.add(new Customer(x, "Ju", timeToOrderAndBeServed()));
            }
            if(chance > 0.5 && chance <= 0.75)
            {
               line.add(new Customer(x, "So", timeToOrderAndBeServed()));
            }
            if(chance <= 1 && chance > 0.75)
            {
               line.add(new Customer(x, "Fr", timeToOrderAndBeServed()));
            }
            
            displayTimeAndQueue(x, line);
            if(line.size() == 1)
               thisCustomersTime = line.peek().getTimeToBeServed();
         }
         else
         {
            displayTimeAndQueue(x, line);           
         ...
         if(thisCustomersTime == 0)
         {
            ...
            int grade = Integer.parseInt(line.peek().getGradeNumber());
            line.remove();
            if(grade == 1)
            {
               seniorWait += x - arrivalTime;
               seniorTotal += 1;
               seniorLongest = Math.max(x - arrivalTime, seniorLongest);
            }
            else if(grade == 2)
            {
               juniorWait += x - arrivalTime;
               juniorTotal += 1;
               juniorLongest = Math.max(x - arrivalTime, juniorLongest);
            }
            else if(grade == 3)
            {
               sophomorWait += x - arrivalTime;
               sophomorTotal += 1;
               sophomorLongest = Math.max(x - arrivalTime, sophomorLongest);
            }
            else if(grade == 4)
            {
               freshmanWait += x - arrivalTime;
               freshmanTotal += 1;
               freshmanLongest = Math.max(x - arrivalTime, freshmanLongest);
            }
            if(line.peek() != null)
               thisCustomersTime = line.peek().getTimeToBeServed();
            else
      ...
      while(!line.isEmpty())
      {
         ...
         if(thisCustomersTime == 0)
         {
            ...
            if(grade == 1)
            {
               seniorWait += 1080 - arrivalTime + timePassed;
               seniorTotal += 1;
               seniorLongest = Math.max(1080 - arrivalTime + timePassed, seniorLongest);
            }
            else if(grade == 2)
            {
               juniorWait += 1080 - arrivalTime + timePassed;
               juniorTotal += 1;
               juniorLongest = Math.max(1080 - arrivalTime + timePassed, juniorLongest);
            }
            else if(grade == 3)
            {
               sophomorWait += 1080 - arrivalTime + timePassed;
               sophomorTotal += 1;
               sophomorLongest = Math.max(1080 - arrivalTime + timePassed, sophomorLongest);
            }
            else if(grade == 4)
            {
               freshmanWait += 1080 - arrivalTime + timePassed;
               freshmanTotal += 1;
               freshmanLongest = Math.max(1080 - arrivalTime + timePassed, freshmanLongest);
            }
         ...
         if(!line.isEmpty())
         {
            displayTimeAndQueue(1079 + timePassed, line);           
         }
      }      
      ...
      System.out.println("Senior\t\t\t" + seniorTotal + "\t\t\t" + seniorLongest + "\t\t\t" + seniorWait/seniorTotal);
      System.out.println("Junior\t\t\t" + juniorTotal + "\t\t\t" + juniorLongest + "\t\t\t" + juniorWait/juniorTotal);
      System.out.println("Sophomor\t\t\t" + sophomorTotal + "\t\t\t" + sophomorLongest + "\t\t\t" + sophomorWait/sophomorTotal);
      System.out.println("Freshman\t\t\t" + freshmanTotal + "\t\t\t" + freshmanLongest + "\t\t\t" + freshmanWait/freshmanTotal);
   ...
   public static class Customer implements Comparable<Customer>
   {
      ...
      public Customer(int timeArrived, String grade, int servingTime){
         arrivalTime = timeArrived;
         rank = grade;
         timeToBeServed = servingTime;
         if(rank == "Se")
            gradeNumber = "1";
         else if(rank == "Ju")
            gradeNumber = "2";
      ...
      public String toString(){
         return arrivalTime + ":" + rank;
      }    
      
      public int compareTo(Customer comp)
      {
         return gradeNumber.compareTo(comp.getGradeNumber());
      }
   }
}
