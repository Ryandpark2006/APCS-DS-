 ...
 //constructors
   ...
   */
   ...
   private int foundOrInserted(String word)
   {
      ...
      {
         if(this.get(x).getWord().equals(word.toUpperCase()))
            return x;
         else if(this.get(x).getWord().compareTo(word.toUpperCase()) > 0)
         {
            this.add(x, new IndexEntry(word));
            return x;
         }
      }
   ...
   //       while(this.getWord().compareTo(word) != 0)
   //       {
   //          
   //          
   //       }
...
class IndexEntry implements Comparable<IndexEntry>
{
   ...
   public String toString()
   {
      ...
      for(int x = 0; x < numsList.size() - 1; x++)
         format = format + numsList.get(x) + ", ";
      format = format + numsList.get(numsList.size() - 1);
      return format;
   }
   
   public int compareTo(IndexEntry n)
   {
      return word.compareTo(n.getWord());   
   }
}
