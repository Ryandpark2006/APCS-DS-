   ...
   */
   public static int[] calculateModes(int[] tally)
   {
      ...
      if(tally.length > 0)
      {
         ...
         {
            if(tally[x] == findMax(tally))                //finds number of modes which is length for array
            {
               count++;
            }
         }
         ...
         int index = 0;
         for(int x = 0; x < tally.length; x++)  //iterate through tally and finds the index which has the max
         {
            if(tally[x] == findMax(tally))
            {
               modes[index] = x;     //adds the index(mode) to the array which will be returned
               index++;   
            }
         }
